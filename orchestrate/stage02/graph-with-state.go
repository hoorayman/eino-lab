...
func OrcGraphWithState() {
	ctx := context.Background()
	input := map[string]string{"role": "cute", "content": "你好啊"}
	...
	TsundereLambda := compose.InvokableLambda(func(ctx context.Context, input map[string]string) (output []*schema.Message, err error) {
		compose.ProcessState(ctx, func(_ context.Context, state *State) error {
	...
	cutePreHandler := func(ctx context.Context, input map[string]string, state *State) (
